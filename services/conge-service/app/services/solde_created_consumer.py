...
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "conge_service.settings")
django.setup()

from app.services.conge_service import CongeService


class SoldeCreatedConsumer:
    def __init__(self):
    ...
    def _on_message(self, channel, method, _properties, body):
        try:
            ...
            conge_id = uuid.UUID(payload["conge_id"])
            solde_id = uuid.UUID(payload["solde_id"])
            self.conge_service.apply_solde_created_event(
                conge_id=conge_id,
                solde_id=solde_id,
            )
