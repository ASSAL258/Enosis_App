  ...
  const rows = demandes.map((item) => ({
    ...
    cells: isRH
      ? [
        ...
        { content: item.attestation_rib_file_id ? (
          <button
            type="button"
            className="pdf-badge"
            onClick={() => openResource(item.attestation_rib_file_id)}
            disabled={!!resourceLoading[item.attestation_rib_file_id]}
            title="Open PDF"
          >
          ...
          <span className="comment-editor__placeholder">Non fourni</span>
        ) },
        { content: item.main_levee_file_id ? (
          <button
            type="button"
            className="pdf-badge"
            onClick={() => openResource(item.main_levee_file_id)}
            disabled={!!resourceLoading[item.main_levee_file_id]}
            title="Open PDF"
          >
            <Icon name="pdf" size={16} />
            <span>PDF</span>
